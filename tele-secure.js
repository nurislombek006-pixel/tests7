(function(){
  ...
  function _info(){
    ...
    const ios=ua.match(/OS ([\d_]+) like Mac OS X/i);
    ...
    if(/iPhone/i.test(ua)){
      ...
      device="iPad";
      os=ios ? "iPadOS " + ios[1].replace(/_/g,'.') : "iPadOS";
    }else if(android){
  ...
  window.sendSecureReport=function(userProfile,correctAnswers,totalQuestions,userId,meta){
    ...
    if(details.length){
      ...
      details.forEach(d=>{
        const num=d.num||d.id||'-';
        const ans=(d.answerIndex===null||d.answerIndex===undefined) ? 'нет ответа' : d.answerIndex;
        const mark=d.isOk?'✅':'❌';
        text+=`\n№${_safe(num)} — ответ ${_safe(ans)} — ${mark}`;
      });
    }
    _send(text);
  };
